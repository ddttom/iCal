...
class CalendarManager {
        ...
        return events.map(event => {
            ...
            return {
                ...
                startDate: {
                    ...
                    timezone: vevent.startDate.zone.toString() === 'floating' ? 'floating' : (vevent.startDate.zone.tzid || 'UTC')
                },
                endDate: vevent.endDate ? {
                    dateTime: vevent.endDate.toString(),
                    timezone: vevent.endDate.zone.toString() === 'floating' ? 'floating' : (vevent.endDate.zone.tzid || 'UTC')
                } : null,
                description: vevent.description,
                location: vevent.location,
                isRecurring: vevent.isRecurring(),
                raw: event.toString()
            };
        });
    }
    ...
    searchEvents(query, startDate = null, endDate = null) {
        ...
        return events.filter(event => {
            ...
            if (startDate) {
                ...
                // For filtering, we still need a comparable Date. 
                // If floating, treat as local (which new Date(iso) does if no Z). 
                // But wait, new Date('2025-12-01T12:00:00') is local.
                // The structured data has dateTime string.
                const endStr = event.endDate ? event.endDate.dateTime : event.startDate.dateTime;
                const eventEnd = new Date(endStr); // This will be local if no Z, or UTC if Z.
            ...
            if (endDate) {
                ...
                const startStr = event.startDate.dateTime;
    ...
    addEvent(eventData) {
        ...
        if (eventData.startDate) {
            // Input is now expected to be ISO string. 
            // If it has no Z, it's floating. If it has Z, it's UTC.
            // We can just pass the string to ICAL.Time.fromString
            event.startDate = ICAL.Time.fromString(eventData.startDate);
        }
        if (eventData.endDate) {
            event.endDate = ICAL.Time.fromString(eventData.endDate);
        }
    ...
    updateEvent(uid, updates) {
        ...
        if (updates.startDate) event.startDate = ICAL.Time.fromString(updates.startDate);
        if (updates.endDate) event.endDate = ICAL.Time.fromString(updates.endDate);
