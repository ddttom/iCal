...
class CalendarManager {
    ...
    async listEvents(limit = 100, offset = 0) {
        const rows = await this.db.getAllEvents(limit, offset);
        return rows.map(this._mapRowToEvent);
    }

    async searchEvents(query, startDate, endDate, limit = 100, offset = 0) {
        const rows = await this.db.searchEvents(query, startDate, endDate, limit, offset);
        return rows.map(this._mapRowToEvent);
    }
