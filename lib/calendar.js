...
class CalendarManager {
    ...
    async addEvent(eventData) {
        ...
        const ensureSeconds = (str) => {
            ...
            const res = str.length === 16 ? str + ':00' : str;
            console.log(`ensureSeconds: '${str}' -> '${res}'`);
            return res;
        };
        ...
        try {
            if (eventData.startDate) event.startDate = ICAL.Time.fromString(ensureSeconds(eventData.startDate));
            if (eventData.endDate) {
                event.endDate = ICAL.Time.fromString(ensureSeconds(eventData.endDate));
            } else {
                 // Default duration 1 hour if not specified
                 const start = ICAL.Time.fromString(ensureSeconds(eventData.startDate));
